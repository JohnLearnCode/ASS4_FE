...
import { fetchQuizzes, deleteQuiz, clearMessages } from '../store/quizSlice';
...
export default function AdminQuizList() {
  ...
  const { quizzes, loading, error, successMessage } = useSelector((s: RootState) => s.quiz);
  ...
  const handleDelete = (id: string, title: string) => {
    if (window.confirm(`Xoá quiz "${title}"?`)) {
    ...
    <div>
      <div className="d-flex align-items-center justify-content-between mb-3">
        <div className="d-flex align-items-center gap-3">
          ...
          <h1 className="fs-4 fw-bold mb-0">⚙️ Quản lý Quiz</h1>
        </div>
        <button className="btn btn-warning text-white btn-sm fw-medium" onClick={() => { dispatch(clearMessages()); navigate('/admin/quizzes/create'); }}>
          + Tạo Quiz
        </button>
      </div>
      ...
      {!loading && (
        <div className="position-relative mb-3" style={{ maxWidth: 360 }}>
          ...
          <input className="form-control ps-5" type="text" placeholder="Tìm kiếm quiz theo tiêu đề..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-5 text-muted">
          ...
          {search ? (
            ...
            <button className="btn btn-warning text-white btn-sm mt-2" onClick={() => navigate('/admin/quizzes/create')}>+ Tạo Quiz</button>
          )}
        </div>
      ) : (
        <>
              ...
              <div key={q._id} className="card border admin-quiz-card">
                <div className="card-body d-flex align-items-center gap-3">
                  ...
                  <div className="d-flex gap-1 flex-shrink-0">
                    <button className="btn btn-outline-primary btn-sm" onClick={() => navigate(`/admin/quizzes/${q._id}/edit`)}>✏️ Sửa</button>
                    <button className="btn btn-outline-success btn-sm" onClick={() => navigate(`/admin/quizzes/${q._id}/edit?addQuestion=1`)}>+ Câu hỏi</button>
